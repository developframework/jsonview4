 ...
 * @date 2017/5/9
 */
public class OneToOnePropertyProcessor extends NormalPropertyProcessor implements OneToOneProcessor{

    @Getter
    private int index;

    public OneToOnePropertyProcessor(ProcessContext processContext, PropertyElement element, Expression parentExpression) {
    ...
    public void checkSize(int parentArraySize) {
        ...
        if (objOptional.isPresent()) {
            ...
            if (size != parentArraySize) {
                throw new OneToOneSizeNotEqualException("property-one-to-one", element.getNamespace(), element.getTemplateId());
            }
        }
    }
